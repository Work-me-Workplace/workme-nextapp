...
export default function EmailDigestEditionPage({
  ...
  useEffect(() => {
    async function fetchEdition() {
      const result = await getEmailDigestEdition(editionId)
    ...
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center">
        <div className="text-center">
          ...
          <Link href={`/workforce/enduring/email-digest/${emailDigestId}`} className="text-blue-600 hover:underline">
            Back to Product
          </Link>
        </div>
      </div>
  ...
  const content = edition.contentJson as any
            ...
            <div className="space-y-4">
              ...
              {content.body && (
                <div>
                  ...
                  <div className="bg-gray-50 rounded-lg p-4">
                    <pre className="whitespace-pre-wrap text-gray-900">{content.body}</pre>
                  </div>
                </div>
              )}
            </div>
