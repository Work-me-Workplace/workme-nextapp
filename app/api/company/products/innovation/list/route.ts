...
export async function GET(request: Request) {
  try {
    ...
    // TODO: Add companyId field to CompanyProductInnovation schema and filter by it
    // For now, we validate companyId but return all products (schema update needed)
    const products = await prisma.companyProductInnovation.findMany({
      orderBy: { createdAt: 'desc' },
    })

    console.log(`[API GET /api/company/products/innovation/list] Returning ${products.length} products for companyId: ${companyId}`)
    ...
    console.error('Failed to list innovation products:', error)
