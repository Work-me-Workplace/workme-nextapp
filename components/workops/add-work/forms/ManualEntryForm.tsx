...
export default function ManualEntryForm({ onSubmit, loading }: ManualEntryFormProps) {
  ...
  const [derivedFrom, setDerivedFrom] = useState<WorkOpsDerivedFrom>(WorkOpsDerivedFrom.my_own)
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6">
      ...
      <div>
        <label htmlFor="derivedFrom" className="block text-sm font-medium text-gray-700 mb-2">
          Where is this from?
        </label>
        <select
          id="derivedFrom"
          value={derivedFrom}
          onChange={(e) => setDerivedFrom(e.target.value as WorkOpsDerivedFrom)}
          className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
        >
