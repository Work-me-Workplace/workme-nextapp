...
export interface ApolloPersonResponse {
  person?: {
    ...
    // Apollo sometimes nests data differently
    [key: string]: any;
  };
}

/**
 * Parse Apollo person response and extract ALL available data
 * Handles various Apollo response structures and extracts everything possible
 */
export function parseApolloPersonResponse(apolloData: ApolloPersonResponse): {
  fullName?: string;
  firstName?: string;
  lastName?: string;
  email?: string;
  ...
  if (person.name && person.name.trim()) {
    parsed.fullName = person.name.trim();
  } else if (person.first_name || person.last_name) {
    parsed.fullName = `${person.first_name || ''} ${person.last_name || ''}`.trim();
    parsed.firstName = person.first_name;
    parsed.lastName = person.last_name;
  }
  ...
  if (person.email) {
    ...
    // Apollo sometimes returns emails array
    parsed.email = person.emails[0].email || person.emails[0];
  }

  // Extract title
  if (person.title) {
    parsed.title = person.title;
  } else if (person.employment_history && person.employment_history.length > 0) {
    // Try to get title from current employment
    const currentJob = person.employment_history.find((job: any) => !job.ended_at && !job.end_date);
    if (currentJob?.title) {
      parsed.title = currentJob.title;
    }
  }
  ...
  if (person.phone_numbers && person.phone_numbers.length > 0) {
    ...
    parsed.phone = phone.sanitized_number || phone.raw_number;
  }

  // Extract LinkedIn URL
  if (person.linkedin_url) {
    parsed.linkedinUrl = person.linkedin_url;
  }

  // Extract company name
  if (person.organization?.name) {
    parsed.companyName = person.organization.name;
  } else if (person.employment_history && person.employment_history.length > 0) {
    const currentJob = person.employment_history.find((job: any) => !job.ended_at && !job.end_date);
    if (currentJob?.organization_name) {
      parsed.companyName = currentJob.organization_name;
    } else if (currentJob?.organization?.name) {
      parsed.companyName = currentJob.organization.name;
    } else if (currentJob?.company_name) {
      parsed.companyName = currentJob.company_name;
    }
  }
